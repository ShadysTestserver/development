local whitelistedgangs = {}
RegisterServerEvent("sts:whitelistgang")
AddEventHandler("sts:whitelistgang", function(gangs)
	whitelistedgangs = {}
	whitelistedgangs = gangs
end)
...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords)
	...
	self.forceaddInventoryItem = function(name, count)
		local item = self.getInventoryItem(name)
		...
		if item then
			...
			TriggerEvent('esx:onAddInventoryItem', self.source, item.name, item.count)
			self.triggerEvent('esx:forceaddInventoryItem', item.name, item.count)
		end
	end
		...
		if item and count >= 0 then
			...
			if count > item.count then
				...
				if item.name == 'gps' then
					TriggerEvent('esx_gps:addGPS', source)
				end
			else
				self.removeInventoryItem(item.name, item.count - count)
				if item.name == 'gps' and item.count < 1 then
					TriggerEvent('esx_gps:removeGPS', source)
				end
			end
		end
	end
	...
	self.setJob = function(job, grade, whitelisted)
		if whitelisted == false then
			for key, data in pairs(whitelistedgangs) do
				if tostring(job) == tostring(key) or tostring(job) == "admin" then
					TriggerClientEvent('esx:showNotification', self.source, "Deze gang is ~b~whitelisted~s~! Bekijk onze ~g~tebex ~s~voor een custom gang!")
					return
				end
			end
		end
  ...
  ---SECONDJOB INCLUDED
	self.setJob2 = function(job2, grade2, whitelisted)
		for key, data in pairs(whitelistedgangs) do
			if tostring(job2) == tostring(key) or tostring(job) == "admin" then
				TriggerClientEvent('esx:showNotification', self.source, "Deze gang is ~b~whitelisted~s~! Bekijk onze ~g~tebex ~s~voor een custom gang!")
				return
			end
		end
	...
	self.forceaddWeapon = function(weaponName, ammo)
		if not self.hasWeapon(weaponName) then
			local weaponLabel = ESX.GetWeaponLabel(weaponName)
			...
			self.triggerEvent('esx:forceaddWeapon', weaponName, ammo)
			self.triggerEvent('esx:forceaddInventoryItem', weaponLabel, false, true)
		end
	end
	...
	self.forceaddWeaponComponent = function(weaponName, weaponComponent)
		local loadoutNum, weapon = self.getWeapon(weaponName)
		...
		if weapon then
			...
			if component then
				if not self.hasWeaponComponent(weaponName, weaponComponent) then
					table.insert(self.loadout[loadoutNum].components, weaponComponent)
					self.triggerEvent('esx:forceaddWeaponComponent', weaponName, weaponComponent)
					self.triggerEvent('esx:forceaddInventoryItem', component.label, false, true)
				end
			end
		end
	end
	...
	self.removeWeaponUser = function(weaponName)
		local weaponLabel
		...
		if weaponLabel then
			self.triggerEvent('esx:removeWeaponUser', weaponName)
			self.triggerEvent('esx:removeInventoryItemUser', weaponLabel, false, true)
		end
	end
