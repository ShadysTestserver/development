...
ESX.Game.GetObjectsInArea = function(coords, radius, filter)
	...
	if type(filter) == "string" then
		local _filter = GetHashKey(filter)
		filter = _filter
	...
	for k,v in pairs(objects) do
		if not filterIsTable then
			...
			for k2,v2 in pairs(filter) do
				if v2 == GetEntityModel(v) then
					local objCoords = GetEntityCoords(v)
					local distance = #(objCoords - coords)
					if distance < radius then
						foundObjects[#foundObjects + 1] = {handle = v, coords = objCoords, distance = distance}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
				...
				if IsPedOnFoot(playerPed) then
					...
					if type == 'item_weapon' then
						...
						if exports['redzone']:isinredzone() == true then
							ESX.ShowNotification("Je hebt je wapen laten vallen! Probeer de tegenstanders neer te slaan met je vuisten.")
						end
					else
			...
			elseif data1.current.action == 'see' then
				if type == 'license' then
					TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId())) --Spingbob
				end
